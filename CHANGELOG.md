...
A church outside our own found OpenEar, stood it up themselves, and sent a defect
report. First external user, first bug report, and the first time the ship gate has had
anything real to guard.

### Added
- **`docs/bugs.json`** — the tracker. One file, kept alongside the code (not in the
  repository), carrying its own legend. Series are `OE` (bug),
